...
The `GradeStep` reads the `AI_BATCH` step result from the pipeline and passes it as `pre_computed_results` to `GraderService.grade_from_tree()`. The `SubmissionGrader` threads this dict through the tree traversal into every `process_test()` call, which forwards it as a kwarg to `test_function.execute()`.
