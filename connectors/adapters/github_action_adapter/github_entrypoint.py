...
parser.add_argument(
    "--openai-key", type=str, required=False, help="OpenAI API key for AI feedback"
)
parser.add_argument(
    "--redis-url", type=str, required=False, help="Redis URL for AI feedback"
)
parser.add_argument(
    "--redis-token", type=str, required=False, help="Redis token for AI feedback"
)
...
async def main():
    ...
    if feedback_type == "ai":
        if not args.openai_key or not args.redis_url or not args.redis_token:
            raise ValueError(
                "OpenAI key, Redis URL, and Redis token are required for AI feedback."
            )
