...
async def grade_submission_endpoint(
    submission_files: List[UploadFile] = File(
        ..., description="The student's source code files (HTML, CSS, JS)"
    ),
    template_preset: str = Form(
        ...,
        description="The grading preset to use (e.g., api, html, python, etc.) or custom for custom configuration",
    ),
    student_name: str = Form(..., description="The name of the student"),
    student_credentials: str = Form(
        ..., description="The credentials of the student (e.g., GitHub token)"
    ),
    include_feedback: bool = Form(
        False, description="Whether to include detailed feedback in the response"
    ),
    feedback_type: Optional[str] = Form(
        "default", description="The type of feedback to provide (default or ai)"
    ),
    custom_template: Optional[UploadFile] = File(
        None,
        description="A python file with custom tests that follows the template structure (in case of custom preset)",
    ),
    criteria_json: UploadFile = File(
        None, description="JSON file with grading criteria (in case of custom preset)"
    ),
    feedback_json: UploadFile = File(
        None,
        description="JSON file with feedback configuration (in case of custom preset)",
    ),
    setup_json: Optional[UploadFile] = File(
        None,
        description="JSON file with commands configuration (in case of custom preset)",
    ),
    openai_key: Optional[str] = Form(
        None, description="OpenAI API key for AI feedback"
    ),
    ...
    try:
        ...
        if template_preset == "custom":
            ...
            if not custom_template:
                raise HTTPException(
                    status_code=400, detail="Custom template file not given."
                )
            assignment_config = await adapter.load_assignment_config(
                template="custom",
            ...
            logging.info(
                f"Using preset: {template_preset}. Loading template preset configuration."
            )
            assignment_config = await adapter.load_assignment_config(
        ...
        logging.info("Creating autograder request....")
        ...
        logging.info("Autograder request created successfully.")
