...
from autograder.models.dataclass.step_result import StepName, StepStatus, StepResult
...
def test_cleanup_sandbox_called_after_pipeline_run(mock_get_manager, submission, pipeline_exec):
...
@patch("sandbox_manager.manager.get_sandbox_manager")
def test_cleanup_sandbox_even_on_step_failure(mock_get_manager, submission, pipeline_exec):
    mock_manager = mock_get_manager.return_value
    mock_sandbox = MagicMock(spec=SandboxContainer)
...
@patch("sandbox_manager.manager.get_sandbox_manager")
def test_cleanup_no_sandbox_no_call(mock_get_manager, submission, pipeline_exec):
    mock_manager = mock_get_manager.return_value
