...
let axiosWithRedicrect = axios.create({ maxRedirects: 5 });

/**
 * Tests for the root route.
 * 1) Tests status code and headers (Should be 200 and content-type html)
 * 2) Tests the HTML content: should have at least one form with a submission button that makes either
 *    a GET or POST request to /suggestions (Response can either be a redirect to /obrigado the orbigado.html
 *    page)
 */
describe('Route - /', () => {
  let response;
...
function testContatoRecebidoPageContent($, submittedData) {
    describe('HTML Content Analysis of contato-recebido.html', () => {

      test('should display the submitted "nome"', () => {
            expect($.html()).toContain(submittedData.nome);
        });

        test('should display the submitted "email"', () => {
            expect($.html()).toContain(submittedData.email);
        });

        test('should display the submitted "assunto"', () => {
            expect($.html()).toContain(submittedData.assunto);
        });

        test('should display the submitted "mensagem"', () => {
            expect($.html()).toContain(submittedData.mensagem);
        });
...
describe('Route - /contato (POST)', () => {
  const contactSubmission = {
    ...
    describe('HTML Content Analysis (Final Page)', () => {
        beforeAll(() => {
            if (!($ && finalResponse && finalResponse.headers['content-type'] && finalResponse.headers['content-type'].includes('html'))) {
                pending('Skipping HTML content tests as the final response was not HTML or Cheerio failed to load.');
            }
        });
        testContatoRecebidoPageContent($, contactSubmission);
    });
