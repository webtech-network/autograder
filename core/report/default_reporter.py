...
class DefaultReporter(BaseReporter):
    ...
    def generate_feedback(self,feedback_file="feedback.json"):
        ...
        # Initialize feedback
        feedback = f"# 🧪 Relatório de Avaliação – Autograder HTML - {self.result.author}\n\n"
        ...
        if len(self.result.base_results["failed"]) == 0:
            ...
            for test_name in self.results.base_results["failed"]:
        ...
        if len(self.result.bonus_results["passed"]) > 0:
            feedback += f"- Você conquistou `{len(self.result.bonus_result['passed'])}` bônus! Excelente trabalho nos detalhes adicionais!\n"
            for passed_test in self.result.bonus_result["passed"]:
