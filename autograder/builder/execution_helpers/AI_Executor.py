...
class TestOutput(BaseModel):
    ...
    feedback: str = Field(
        ...,
        description="Description of why and how this response was achieved, pinpointing specific parts of the prompt that lead you to that conclusion.",
    )
    subject: str = Field(
        ...,
        description="The subject of the test, usually specified at the beginning of its title before a colon",
    )
    score: float = Field(
        ...,
        description="Based on whether the submission follows this correction criterion and how well it does, grade it from 0 to 100",
    )
...
class AIResponseModel(BaseModel):
    ...
    results: List[TestOutput] = Field(
        description="A list of test results each test performed following the TestOutput format"
    )
...
class AiExecutor:
    ...
    def add_test(self, test_name: str, test_prompt: str):
        ...
        test_input_model = TestInput(test_name=test_name, prompt=test_prompt)
        self.tests.append(test_input_model)
        empty_test_result = TestResult(
            test_name, 0, "", ""
        )  # TODO -> how to map to correct subject?
    ...
    def mapback(self):
        ...
        for ai_result in self.test_results:
            ...
            matching_refs = [
                ref
                for ref in self.test_result_references
                if ref.test_name == ai_result.title
            ]
            if matching_refs:
                ref = matching_refs[0]
                ...
                print(
                    f"Mapped AI result '{ai_result.title}' with score {ai_result.score} to TestResult."
                )
            else:
                print(
                    f"No matching TestResult found for AI result '{ai_result.title}'."
                )
    ...
    def _create_test_batch(self):
        ...
        test_batch_list = [
            {"test": test.test_name, "prompt": test.prompt} for test in self.tests
        ]
...
In conclusion, AI is a double-edged sword. Its potential to enhance productivity and create new avenues for growth is undeniable, but so are the risks of displacement and inequality. The future of work will not be a battle of humans versus machines, but rather a story of adaptation. By investing in education, promoting equitable policies, and fostering a culture of continuous learning, we can harness the power of AI to build a more prosperous and inclusive workforce for all."""
    ...
    ai_executor.add_test(
        "Content: Identify Specific Examples",
        "In a scale of 0 to 100, how well does the text provide specific examples to support its main points? Consider the relevance and clarity of the examples given.",
    )
    ai_executor.add_test(
        "Clarity: Evaluate Overall Clarity",
        "On a scale from 0 to 100, how clear and understandable is the text? Consider the organization of ideas, sentence structure, and use of language.",
    )
    ai_executor.add_test(
        "Grammar: Check for Grammatical Errors",
        "On a scale from 0 to 100, how free is the text from grammatical errors? Consider issues such as subject-verb agreement, punctuation, and sentence fragments.",
    )
    ai_executor.add_test(
        "Engagement: Assess Reader Engagement",
        "On a scale from 0 to 100, how engaging is the text? Consider the use of anecdotes, rhetorical questions, and vivid language that captures the reader's interest.",
    )
