...
class HasClass(TestFunction):
    ...
    def execute(
        self, html_content: str, class_names: list[str], required_count: int
    ) -> TestResult:
        ...
        score = (
            min(100, int((found_count / required_count) * 100))
            if required_count > 0
            else 100
        )
...
class CheckBootstrapLinked(TestFunction):
    ...
    @property
    def parameter_description(self):
        return {"html_content": "O conteúdo HTML a ser analisado."}

    def execute(self, html_content: str) -> TestResult:
        soup = BeautifulSoup(html_content, "html.parser")
        found = (
            soup.find("link", href=re.compile(r"bootstrap", re.IGNORECASE)) is not None
            or soup.find("script", src=re.compile(r"bootstrap", re.IGNORECASE))
            is not None
        )
        score = 100 if found else 0
        report = (
            "O framework Bootstrap foi encontrado no seu HTML."
            if found
            else "Não foi encontrado um link para o CSS ou JS do Bootstrap."
        )
...
class CheckInternalLinks(TestFunction):
    ...
    def parameter_description(self):
        return {
            "html_content": "O conteúdo HTML a ser analisado.",
            "required_count": "O número mínimo de links válidos.",
        }
    ...
    def execute(self, html_content: str, required_count: int) -> TestResult:
        ...
        for link in links:
            ...
            if soup.find(id=target_id):
                valid_links += 1
        score = (
            min(100, int((valid_links / required_count) * 100))
            if required_count > 0
            else 100
        )
        report = f"Encontrados {valid_links} de {required_count} links internos válidos ('âncoras')."
        return TestResult(
            self.name, score, report, parameters={"required_count": required_count}
        )
...
class HasForbiddenTag(TestFunction):
    ...
    def parameter_description(self):
        return {
            "html_content": "O conteúdo HTML a ser analisado.",
            "tag": "A tag HTML proibida a ser pesquisada.",
        }
    ...
    def execute(self, html_content: str, tag: str) -> TestResult:
        ...
        report = (
            f"A tag `<{tag}>` foi encontrada e é proibida."
            if found
...
class HasAttribute(TestFunction):
    ...
    def execute(
        self, html_content: str, attribute: str, required_count: int
    ) -> TestResult:
        soup = BeautifulSoup(html_content, "html.parser")
...
class CheckNoUnclosedTags(TestFunction):
    ...
    @property
    def description(self):
        return "Realiza uma verificação básica para um documento HTML bem formado."
...
class CheckNoInlineStyles(TestFunction):
    ...
    def execute(self, html_content: str) -> TestResult:
        found_count = len(
            BeautifulSoup(html_content, "html.parser").find_all(style=True)
        )
        score = 0 if found_count > 0 else 100
        report = (
            f"Foi encontrado {found_count} inline styles (`style='...'`). Mova todas as regras de estilo para seu arquivo `.css`."
            if found_count > 0
            else "Excelente! Nenhum estilo inline foi encontrado."
        )
        return TestResult(self.name, score, report)
...
class CheckCssLinked(TestFunction):
    ...
    def execute(self, html_content: str) -> TestResult:
        ...
        score = 100 if found else 0
        report = (
            "Arquivo CSS está corretamente linkado com o HTML."
            if found
...
class CssUsesProperty(TestFunction):
    ...
    def parameter_description(self):
        return {
            "css_content": "O conteúdo CSS a ser analisado.",
            "prop": "A propriedade CSS.",
            "value": "O valor esperado.",
        }

    def execute(self, css_content: str, prop: str, value: str) -> TestResult:
        pattern = re.compile(
            rf"{re.escape(prop)}\s*:\s*.*{re.escape(value)}", re.IGNORECASE
        )
        found = pattern.search(css_content) is not None
        score = 100 if found else 0
        report = (
            f"A propriedade `{prop}: {value};` foi encontrada."
            if found
            else f"A propriedade CSS `{prop}: {value};` não foi encontrada."
        )
        return TestResult(
            self.name, score, report, parameters={"prop": prop, "value": value}
        )
...
class CountOverUsage(TestFunction):
    ...
    def parameter_description(self):
        return {
            "css_content": "O conteúdo CSS a ser analisado.",
            "text": "O texto a ser contado.",
            "max_allowed": "O número máximo de ocorrências permitidas.",
        }
    ...
    def execute(self, css_content: str, text: str, max_allowed: int) -> TestResult:
        ...
        report = (
            f"Uso exagerado de `{text}` detectado {found_count} vezes (máximo permitido: {max_allowed})."
            if score == 0
...
class JsUsesFeature(TestFunction):
    @property
    def name(self):
        return "js_uses_feature"

    @property
    def description(self):
        return "Realiza uma busca de string simples para verificar se uma funcionalidade específica está presente."

    @property
    def parameter_description(self):
        return {
            "js_content": "O conteúdo JavaScript a ser analisado.",
            "feature": "A funcionalidade a ser pesquisada.",
        }
    ...
    def execute(self, js_content: str, feature: str) -> TestResult:
        ...
        score = 100 if found else 0
        report = (
            f"A funcionalidade `{feature}` foi implementada."
            if found
            else f"A funcionalidade JavaScript `{feature}` não foi encontrada no seu código."
        )
...
class UsesForbiddenMethod(TestFunction):
    ...
    def parameter_description(self):
        return {
            "js_content": "O conteúdo JavaScript a ser analisado.",
            "method": "O nome do método proibido.",
        }
    ...
    def execute(self, js_content: str, method: str) -> TestResult:
        ...
        score = 0 if found else 100
        report = (
            f"Penalidade: Método proibido `{method}()` detectado."
            if found
            else f"Ótimo! O método proibido `{method}()` não foi usado."
        )
...
class CountGlobalVars(TestFunction):
    ...
    def parameter_description(self):
        return {
            "js_content": "O conteúdo JavaScript a ser analisado.",
            "max_allowed": "O número máximo de variáveis globais permitidas.",
        }

    def execute(self, js_content: str, max_allowed: int) -> TestResult:
        found_count = len(
            re.findall(r"^\s*(var|let|const)\s+", js_content, re.MULTILINE)
        )
        score = 100 if found_count <= max_allowed else 0
        report = (
            f"Atenção: {found_count} variáveis globais detectadas (máximo permitido: {max_allowed})."
            if score == 0
            else "Bom trabalho mantendo o escopo global limpo."
        )
        return TestResult(
            self.name, score, report, parameters={"max_allowed": max_allowed}
        )
...
class CheckHeadingsSequential(TestFunction):
    ...
    @property
    def description(self):
        return "Verifica se os níveis de cabeçalho são sequenciais e não pulam níveis."
    ...
    def execute(self, html_content: str) -> TestResult:
        ...
        is_sequential = all(
            headings[i] <= headings[i + 1] for i in range(len(headings) - 1)
        )
...
class CheckAllImagesHaveAlt(TestFunction):
    ...
    def execute(self, html_content: str) -> TestResult:
        ...
        if not images:
            return TestResult(
                self.name, 100, "Nenhuma imagem encontrada para verificar."
            )
        with_alt = sum(
            1 for img in images if img.has_attr("alt") and img["alt"].strip()
        )
...
class CheckHtmlDirectChildren(TestFunction):
    ...
    def execute(self, html_content: str) -> TestResult:
        ...
        children_names = [
            child.name for child in html_tag.findChildren(recursive=False) if child.name
        ]
        is_valid = (
            all(name in ["head", "body"] for name in children_names)
            and "head" in children_names
            and "body" in children_names
        )
        report = (
            "Estrutura da tag <html> está correta."
            if is_valid
            else "A tag <html> deve conter apenas as tags <head> e <body> como filhos diretos."
        )
...
class CheckTagNotInside(TestFunction):
    ...
    def parameter_description(self):
        return {
            "html_content": "O conteúdo HTML a ser analisado.",
            "child_tag": "A tag filha.",
            "parent_tag": "A tag pai.",
        }
    ...
    def execute(self, html_content: str, child_tag: str, parent_tag: str) -> TestResult:
        ...
        report = (
            f"A tag `<{child_tag}>` não deve ser usada dentro da tag `<{parent_tag}>`."
            if found
            else f"A tag `<{child_tag}>` não foi encontrada dentro da tag `<{parent_tag}>`."
        )
        return TestResult(
            self.name,
            0 if found else 100,
    ...
    def execute(self, html_content: str, required_count: int) -> TestResult:
        if not html_content:
            return TestResult(
                self.name,
                0,
                "Arquivo home.html não encontrado.",
                parameters={"required_count": required_count},
            )
        soup = BeautifulSoup(html_content, "html.parser")
        links = soup.select('a[href^="#"]')
...
class HasStyle(TestFunction):
    ...
    def parameter_description(self):
        return {
            "css_content": "O conteúdo CSS a ser analisado.",
            "style": "A regra de estilo.",
            "count": "O número mínimo de ocorrências.",
        }

    def execute(self, css_content: str, style: str, count: int) -> TestResult:
        found_count = len(
            re.findall(rf"{re.escape(style)}\s*:\s*[^;]+;", css_content, re.IGNORECASE)
        )
        score = min(100, int((found_count / count) * 100)) if count > 0 else 100
        report = (
            f"Encontrados {found_count} de {count} `{style}` regras de estilização."
        )
        return TestResult(
            self.name,
            score,
...
class CheckHeadDetails(TestFunction):
    ...
    def parameter_description(self):
        return {
            "html_content": "O conteúdo HTML a ser analisado.",
            "detail_tag": "A tag a ser verificada.",
        }
    ...
    def execute(self, html_content: str, detail_tag: str) -> TestResult:
        ...
        score = 100 if found else 0
        report = (
            f"A tag `<{detail_tag}>` foi encontrada na seção `<head>`."
            if found
            else f"A tag `<{detail_tag}>` não foi encontrada na seção `<head>`."
        )
        return TestResult(
            self.name, score, report, parameters={"detail_tag": detail_tag}
        )
...
class CheckAttributeAndValue(TestFunction):
    ...
    def parameter_description(self):
        return {
            "html_content": "O conteúdo HTML a ser analisado.",
            "tag": "A tag.",
            "attribute": "O atributo.",
            "value": "O valor esperado.",
        }

    def execute(
        self, html_content: str, tag: str, attribute: str, value: str
    ) -> TestResult:
        soup = BeautifulSoup(html_content, "html.parser")
        elements = soup.find_all(tag, attrs={attribute: value})
        score = 100 if elements else 0
        report = (
            f"Encontrada a tag `<{tag}>` com `{attribute}='{value}'`."
            if score == 100
            else f"Não foi encontrada a tag `<{tag}>` com `{attribute}='{value}'`."
        )
        return TestResult(
            self.name,
            score,
...
class CheckDirExists(TestFunction):
    ...
    def parameter_description(self):
        return {
            ...
            "dir_path": "O caminho do diretório.",
        }

    def execute(self, submission_files: dict, dir_path: str) -> TestResult:
        exists = any(
            f.startswith(dir_path.rstrip("/") + "/") for f in submission_files.keys()
        )
        score = 100 if exists else 0
        report = (
            f"O diretório '{dir_path}' existe."
            if exists
...
class CheckProjectStructure(TestFunction):
    ...
    def description(self):
        return (
            "Verifica se o caminho da estrutura esperada existe nos arquivos de envio."
        )
    ...
    def parameter_description(self):
        return {
            "submission_files": "O dicionário de arquivos enviados.",
            "expected_structure": "O caminho do arquivo esperado.",
        }
    ...
    def execute(self, submission_files: dict, expected_structure: str) -> TestResult:
        ...
        score = 100 if exists else 0
        report = (
            f"O arquivo '{expected_structure}' existe."
            if exists
...
class CheckIdSelectorOverUsage(TestFunction):
    ...
    @property
    def description(self):
        return "Conta o número de seletores de ID usados e penaliza se exceder o máximo permitido."

    @property
    def parameter_description(self):
        return {
            "css_content": "O conteúdo CSS a ser analisado.",
            "max_allowed": "O número máximo de seletores de ID permitidos.",
        }
    ...
    def execute(self, css_content: str, max_allowed: int) -> TestResult:
        ...
        score = 100 if found_count <= max_allowed else 0
        report = (
            f"{found_count} seletores de ID detectados (limite: {max_allowed})."
            if score == 0
...
class UsesRelativeUnits(TestFunction):
    ...
    def description(self):
        return (
            "Verifica se o arquivo CSS usa unidades relativas como em, rem, %, vh, vw."
        )

    @property
    def parameter_description(self):
        return {"css_content": "O conteúdo CSS a ser analisado."}

    def execute(self, css_content: str) -> TestResult:
        found = re.search(r"\b(em|rem|%|vh|vw)\b", css_content) is not None
        score = 100 if found else 0
        report = (
            "Estão sendo utilizadas medidas relativas no CSS."
            if found
...
class CheckMediaQueries(TestFunction):
    ...
    def execute(self, css_content: str) -> TestResult:
        ...
        score = 100 if found else 0
        report = (
            "Media queries estão sendo utilizadas no CSS."
            if found
...
class CheckFlexboxUsage(TestFunction):
    ...
    def execute(self, css_content: str) -> TestResult:
        ...
        score = 100 if found else 0
        report = (
            "Propriedades `flexbox` estão sendo utilizadas no CSS."
            if found
...
class CheckBootstrapUsage(TestFunction):
    ...
    def execute(self, html_content: str) -> TestResult:
        ...
        score = 0 if found else 100
        report = (
            "Você está usando bootstrap no seu CSS."
            if found
...
class LinkPointsToPageWithQueryParam(TestFunction):
    ...
    def execute(
        self, html_content: str, target_page: str, query_param: str, required_count: int
    ) -> TestResult:
        soup = BeautifulSoup(html_content, "html.parser")
        ...
        score = (
            min(100, int((valid_link_count / required_count) * 100))
            if required_count > 0
            else 100
        )
...
class JsUsesQueryStringParsing(TestFunction):
    ...
    def execute(self, js_content: str) -> TestResult:
        ...
        score = 100 if found else 0
        report = (
            "O código JavaScript implementa a leitura de parâmetros da URL."
            if found
            else "Não foi encontrada a lógica para ler parâmetros da URL (ex: URLSearchParams) no seu JavaScript."
        )
        return TestResult(self.name, score, report)
...
class JsHasJsonArrayWithId(TestFunction):
    ...
    def execute(self, js_content: str, required_key: str, min_items: int) -> TestResult:
        ...
        match = re.search(
            r"(?:var|let|const)\s+\w+\s*=\s*(\[.*?\]);?", js_content, re.DOTALL
        )
        if not match:
            return TestResult(
                self.name,
...
class JsUsesDomManipulation(TestFunction):
    ...
    def execute(
        self, js_content: str, methods: list, required_count: int
    ) -> TestResult:
        found_count = 0
...
class HasNoJsFramework(TestFunction):
    ...
    def execute(self, submission_files, html_file: str, js_file: str) -> TestResult:
        ...
        report = (
            "Uso de framework JavaScript (React, Vue, Angular) detectado. O uso de frameworks não é permitido nesta atividade."
            if found
            else "Ótimo! Nenhum framework JavaScript proibido foi detectado."
        )
        return TestResult(self.name, score, report)
