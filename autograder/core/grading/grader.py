...
class Grader:
    ...
    def _run(self, submission_files: Dict) -> float:
        ...
        base_score = (
            self._grade_subject_or_category(
                self.criteria.base, submission_files, self.base_results
            )
            or 0.0
        )
        bonus_score = (
            self._grade_subject_or_category(
                self.criteria.bonus, submission_files, self.bonus_results
            )
            or 0.0
        )
        penalty_points = (
            self._calculate_penalty_points(
                self.criteria.penalty, submission_files, self.penalty_results
            )
            or 0.0
        )
        ...
        final_score = self._calculate_final_score(
            base_score, bonus_score, penalty_points
        )
    ...
    def _grade_subject_or_category(
        self,
        current_node: "Subject" or "TestCategory",
        ...
        if hasattr(current_node, "tests") and current_node.tests:
            ...
            for test in current_node.tests:
                test_results = test.get_result(
                    self.test_library, submission_files, current_node.name
                )
        ...
        child_scores_map = {
            sub.name: self._grade_subject_or_category(
                sub, submission_files, results_list, depth + 1
            )
        ...
        valid_children = [
            sub for sub in child_subjects if child_scores_map[sub.name] is not None
        ]
    ...
    def _calculate_penalty_points(
        ...
        return self._calculate_subject_penalty(
            penalty_category, submission_files, results_list, depth=0
        )

    def _calculate_subject_penalty(
        self,
        subject: "Subject",
        submission_files: Dict,
        results_list: List["TestResult"],
        ...
        if hasattr(subject, "tests") and subject.tests:
            ...
            for test in subject.tests:
                test_results = test.get_result(
                    self.test_library, submission_files, subject.name
                )
                if not test_results:
                    continue
                results_list.extend(test_results)
                # Penalty incurred = 100 - score
                penalty_incurred = 100 - sum(res.score for res in test_results) / len(
                    test_results
                )
            ...
            print(
                f"{prefix}  -> Average penalty for '{subject.name}': {avg_penalty_for_subject:.2f}"
            )
        ...
        child_penalties_map = {
            sub.name: self._calculate_subject_penalty(
                sub, submission_files, results_list, depth + 1
            )
            for sub in child_subjects
        }
        ...
        valid_children = [
            sub for sub in child_subjects if child_penalties_map[sub.name] is not None
        ]
    ...
    def _calculate_final_score(
        self, base_score: float, bonus_score: float, penalty_points: float
    ) -> float:
        """
        ...
        print(
            f"  Score Before Penalty: {min(100.0, final_score + penalty_points_to_subtract):.2f}"
        )
