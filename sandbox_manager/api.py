...
async def acquire_sandbox(language: Language):
    manager = get_sandbox_manager()
    try:
...
async def prepare_workdir(sandbox_id: str, request: PrepareWorkdirRequest):
...
async def inject_assets(sandbox_id: str, request: InjectAssetsRequest):
    sandbox = _get_sandbox_or_404(sandbox_id)
    try:
        resolved_assets = [
            ResolvedAsset(
...
async def run_batch(sandbox_id: str, request: RunBatchRequest):
    sandbox = _get_sandbox_or_404(sandbox_id)
    try:
...
async def extract_file(sandbox_id: str, path: str, max_bytes: int = 1_048_576):
    sandbox = _get_sandbox_or_404(sandbox_id)
    try:
...
async def make_request(sandbox_id: str, request: MakeRequestRequest):
    sandbox = _get_sandbox_or_404(sandbox_id)
    try:
...
async def release_sandbox(sandbox_id: str):
    sandbox = active_sandboxes.pop(sandbox_id, None)
    if not sandbox:
        return {"status": "success", "message": "Sandbox not found in active API tracking"}
